...
class Minesweeper2():
    ...
    def nearby_mines(self, cell):
        ...
        return count
    def probability_based_move(self):
        """
        Returns a move to make on the Minesweeper board using a probability-based approach.
        The move is selected based on the likelihood of a cell being a mine.
        """
        all_moves = set(itertools.product(range(self.height), range(self.width)))

        # Remove cells that are already marked as mines or safes
        available_moves = all_moves - self.mines - self.safes

        # Calculate probabilities for each available cell
        probabilities = {}
        for cell in available_moves:
            mine_probability = self.calculate_mine_probability(cell)
            probabilities[cell] = mine_probability

        # Sort cells by probability (ascending order)
        sorted_moves = sorted(probabilities, key=probabilities.get)

        # Choose the cell with the lowest probability (least likely to be a mine)
        return sorted_moves[0] if sorted_moves else None

    def calculate_mine_probability(self, cell):
        """
        Calculates the probability of a cell being a mine based on neighboring cells.
        """
        safe_neighbors = set(cell for cell in self.get_neighbors(cell) if cell in self.safes)
        remaining_mines = self.knowledge_count - len(self.mines)

        if remaining_mines == 0:
            return 0

        probability = remaining_mines / len(safe_neighbors)
        return probability
    ...
    def make_advanced_move(self):
        """
        Returns a move to make on the Minesweeper board using an advanced approach.
        Chooses the next move based on the probability-based method if available,
        otherwise falls back to the original random move method.
        """
        advanced_move = self.probability_based_move()
        return advanced_move if advanced_move else self.make_random_move()
